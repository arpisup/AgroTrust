...
// Test endpoint to verify blockchain connection and batch existence
router.get('/test/connection', async (req, res, next) => {
  try {
    // Test blockchain connection
    const status = await blockchainService.getBlockchainStatus();
    
    // Test if known batches exist
    const testBatches = ['ERD-TUR-2025-001', 'KC-CLOVE-2025-001'];
    const batchResults = [];
    
    for (const batchId of testBatches) {
      try {
        const exists = await blockchainService.contract.checkBatchExists(batchId);
        batchResults.push({
          batchId,
          exists: exists
        });
...
router.get('/list/all', async (req, res, next) => {
  try {
    // For now, return the known batches that should exist
    // In a production system, you would query the blockchain for all batches
    const knownBatches = [
      {
    ...
    // Verify which batches actually exist on the blockchain
    const verifiedBatches = [];
    
    for (const batch of knownBatches) {
      try {
        const exists = await blockchainService.contract.checkBatchExists(batch.batchId);
        if (exists) {
...
router.get('/:batchId', async (req, res, next) => {
  try {
    ...
    const batchInfo = await blockchainService.getBatchInfo(batchId.trim());
...
router.get('/:batchId/exists', async (req, res, next) => {
  try {
    ...
    const exists = await blockchainService.contract.checkBatchExists(batchId.trim());
...
// Get batch summary (basic info only)
router.get('/:batchId/summary', async (req, res, next) => {
  try {
    ...
    const batch = await blockchainService.contract.batches(batchId.trim());
    const farmer = await blockchainService.contract.farmerData(batchId.trim());
...
router.get('/:batchId/transfers', async (req, res, next) => {
  try {
    ...
    const transferCount = await blockchainService.contract.getTransferRecordCount(batchId.trim());
    const transfers = [];
    
    for (let i = 0; i < transferCount; i++) {
      const transfer = await blockchainService.contract.transferLogs(batchId.trim(), i);
      transfers.push({
        index: i,
        from: transfer.from,
        to: transfer.to,
...
// Add new batch data (POST endpoints for data management)
router.post('/add-batch', async (req, res, next) => {
  try {
    ...
    // Convert harvestDate to timestamp if it's a date string
...
router.post('/add-farmer', async (req, res, next) => {
  try {
    ...
    const result = await blockchainService.addFarmerInfo(
      batchId.trim(),
      farmerName.trim(),
      farmLocation.trim(),
      contact.trim(),
...
router.post('/add-transfer', async (req, res, next) => {
  try {
    ...
    // Convert transferDate to timestamp if it's a date string
    const transferTimestamp = typeof transferDate === 'string' 
      ? Math.floor(new Date(transferDate).getTime() / 1000)
      : transferDate;

    const result = await blockchainService.addTransferRecord(
      batchId.trim(),
      from.trim(),
      to.trim(),
      purpose.trim()
...
router.post('/add-cultivation', async (req, res, next) => {
  try {
    ...
    // Convert sowingDate to timestamp if it's a date string
    const sowingTimestamp = typeof sowingDate === 'string' 
      ? Math.floor(new Date(sowingDate).getTime() / 1000)
      : sowingDate;

    const result = await blockchainService.addCultivationDetails(
      batchId.trim(),
      soilType.trim(),
      irrigationType.trim(),
      pesticideUsed.trim(),
...
router.post('/add-processing', async (req, res, next) => {
  try {
    ...
    // Convert processingDate to timestamp if it's a date string
    const processingTimestamp = typeof processingDate === 'string' 
      ? Math.floor(new Date(processingDate).getTime() / 1000)
      : processingDate;

    const result = await blockchainService.addProcessingInfo(
      batchId.trim(),
      processorName.trim(),
      method.trim(),
      processingTimestamp,
      processingUnitId.trim()
    );
...
router.post('/add-lab-result', async (req, res, next) => {
  try {
    ...
    // Convert testDate to timestamp if it's a date string
    const testTimestamp = typeof testDate === 'string' 
      ? Math.floor(new Date(testDate).getTime() / 1000)
      : testDate;

    const resultData = await blockchainService.addLabResult(
      batchId.trim(),
      labName.trim(),
      result.trim(),
      testTimestamp,
...
router.post('/add-certificate', async (req, res, next) => {
  try {
    ...
    // Convert issueDate to timestamp if it's a date string
...
router.post('/add-trace-data', async (req, res, next) => {
  try {
    ...
    const result = await blockchainService.addTraceData(
      batchId.trim(),
      notes.trim(),
      qrCodeHash.trim()
    );
...
// Upload CSV file
router.post('/upload-csv', upload.single('csvFile'), async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        // Convert harvestDate to timestamp
...
router.get('/', async (req, res, next) => {
  try {
    // This would return a list of all batches
    // For now, return a sample response with the batch we created
    return res.json({
      success: true,
      data: {
        batches: [
